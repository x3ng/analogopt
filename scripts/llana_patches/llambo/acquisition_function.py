...
Hyperparameter configuration:"""
    ...
    async def _async_generate_oai(self, user_message):
        '''Generate a response from the LLM async. Uses n=1 with multiple calls
        for compatibility with providers that don't support n > 1.'''
        message = []
        message.append({"role": "system","content": "You are an AI assistant that helps people find information."})
        message.append({"role": "user", "content": user_message})

        class CombinedResponse:
            def __init__(self, choices, total_tokens, prompt_tokens, completion_tokens):
        ...
        async with AsyncOpenAI(
            ...
            for _ in range(self.n_gens):
                try:
                    ...
                    tot_prompt += resp.usage.prompt_tokens
                    tot_completion += resp.usage.completion_tokens
                except Exception as e:
                    raise e

        if not all_choices:
            raise Exception('Response is None')
        self.logger.info(f'Response: {all_choices[0].message.content}')
        combined = CombinedResponse(all_choices, tot_prompt + tot_completion, tot_prompt, tot_completion)
        tot_cost = 0.0015*(tot_prompt/1000) + 0.002*(tot_completion/1000)
        return combined, tot_cost, tot_prompt + tot_completion
                ...
                if self.provider == 'openai':
                    for response_message in response[0].choices:
                            ...
                            try:
                                ...
                                parsed = self._convert_to_json(response_content)
                                if len(parsed) == len(self.task_context['hyperparameter_constraints']):
                                    candidate_points.append(parsed)
                                else:
                                    self.logger.error(f'Incomplete response ({len(parsed)}/{len(self.task_context["hyperparameter_constraints"])} keys): {response_content[:120]}...')
                            except:
