...
> **图表说明**：Panel 1 展示的是"每轮迭代最优 FoM"而非"每次评估 FoM"。BO 每轮 1 次评估，共 10 个 BO 迭代点（不含 10 个 Sobol 初始点）；LLM+BO 每轮评估 3 个候选点（1 BO + 2 LLM），图中仅保留最优者，共 10 个迭代点（不含 10 个 Sobol 初始点 + 每轮被淘汰的 2 个候选点）；RL 则记录了全部 200 个 step 的奖励值。因此 BO 和 LLM+BO 的曲线显得稀疏/平滑，实际评估过程中的波动比图中更大。
